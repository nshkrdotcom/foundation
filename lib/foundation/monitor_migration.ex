defmodule Foundation.MonitorMigration do
  ...
  def generate_migration_report(directory) do
    ...
    issues_by_file = Enum.map(files_with_monitors, fn file ->
      {file, analyze_monitor_usage(file)}
    end)
    
    total_issues = issues_by_file
  ...
  defp replace_monitor_calls(code) do
    ...
    Regex.replace(
      ...
      fn full, pid_expr ->
        ...
        tag = if String.contains?(code, full) do
  ...
  defp add_handle_info_cleanup(code) do
    ...
    if String.contains?(code, "def handle_info({:DOWN") do
      ...
      terminate_pos = case :binary.match(code, "def terminate") do
        {pos, _} -> pos
        :nomatch -> 
  ...
  defp analyze_line(line, line_num) do
    ...
    issues = if String.contains?(line, "Process.monitor") do
      [%{
        line: line_num,
        type: :raw_monitor,
    ...
    issues = if String.contains?(line, "= Process.monitor") and 
  ...
  defp add_structural_issues(line_issues, content) do
    ...
    structural_issues = if String.contains?(content, "use GenServer") and
    ...
    structural_issues = if String.contains?(content, "Process.monitor") and
  ...
  defp generate_report_text(directory, total_files, files_with_monitors, issues_by_file, total_issues) do
    """
