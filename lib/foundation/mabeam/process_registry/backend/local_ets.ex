defmodule Foundation.MABEAM.ProcessRegistry.Backend.LocalETS do
  @moduledoc """
  ...
  @behaviour Foundation.MABEAM.ProcessRegistry.Backend
    ...
    case :ets.info(capability_table) do
      :undefined ->
        :ets.new(capability_table, [
          :named_table,
          :public,
          :bag,  # Multiple agents can have same capability
          read_concurrency: true,
          write_concurrency: true
        ])
      _ ->
  ...
  def find_agents_by_capability(required_capabilities) when is_list(required_capabilities) do
    ...
    if required_capabilities == [] do
      ...
      capability_sets = for capability <- required_capabilities do
      ...
      result = case capability_sets do
        [] -> MapSet.new()
  ...
  def cleanup_inactive_agents do
    ...
    inactive_agents = :ets.tab2list(main_table)
    |> Enum.map(&elem(&1, 1))
  ...
  defp get_capability_table do
    case :ets.info(@table_registry) do
      :undefined -> @capability_index_table
      _ ->
