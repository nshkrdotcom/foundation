defmodule Foundation.CredoChecks.NoProcessDict do
  ...
  if Code.ensure_loaded?(Credo.Check) do
    ...
    defp traverse(
      ...
      issue = format_issue(
        issue_meta,
        message: "Avoid Process.put/2 - use GenServer state, ETS, or explicit parameter passing instead",
        line_no: meta[:line],
        column: meta[:column]
      )
      {ast, [issue | issues]}
    ...
    defp traverse(
      ...
      issue = format_issue(
        issue_meta,
        message: "Avoid Process.get/#{length(args)} - use GenServer state, ETS, or explicit parameter passing instead",
        line_no: meta[:line],
        column: meta[:column]
      )
      {ast, [issue | issues]}
