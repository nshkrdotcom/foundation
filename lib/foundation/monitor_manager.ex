defmodule Foundation.MonitorManager do
  ...
  def handle_call({:monitor, pid, tag, caller}, _from, state) do
    case Process.alive?(pid) do
      ...
      true ->
        ...
        new_reverse_lookup = state.reverse_lookup
        |> Map.put_new(pid, %{})
        |> put_in([pid, ref], true)
        |> Map.put_new(caller, %{})
  ...
  defp handle_caller_down(caller_pid, _ref, state) do
    ...
    # Clean them all up
    new_state = Enum.reduce(monitors_to_clean, state, fn ref, acc_state ->
      case Map.get(acc_state.monitors, ref) do
        ...
        monitor_info ->
          ...
          new_reverse_lookup = update_in(acc_state.reverse_lookup, [monitor_info.pid], &(Map.delete(&1 || %{}, ref)))
