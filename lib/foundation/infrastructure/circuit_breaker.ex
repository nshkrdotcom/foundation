defmodule Foundation.Infrastructure.CircuitBreaker do
  ...
  defp build_fuse_options(circuit_config) do
    ...
    fuse_opts = {{:standard, tolerance, circuit_config.reset_timeout},
                 {:reset, circuit_config.reset_timeout}}
    Logger.debug("Building fuse options: #{inspect(fuse_opts)} (failure_threshold=#{circuit_config.failure_threshold})")
    fuse_opts
  end
      ...
      :ok ->
        ...
        try do
          ...
          exception ->
            ...
            Logger.debug("Function failed, melting fuse #{inspect(service_id)}: #{inspect(exception)}")
          ...
          kind, reason ->
            # Melt the fuse on failure
            Logger.debug("Function crashed, melting fuse #{inspect(service_id)}: #{kind} #{inspect(reason)}")
            :fuse.melt(service_id)
            duration = System.monotonic_time(:millisecond) - start_time
