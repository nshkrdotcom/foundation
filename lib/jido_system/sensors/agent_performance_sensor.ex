defmodule JidoSystem.Sensors.AgentPerformanceSensor do
  ...
  use Jido.Sensor,
    ...
    schema: [
      monitoring_interval: [type: :integer, default: 60_000, 
                           doc: "Performance monitoring interval in milliseconds"],
      performance_thresholds: [type: :map, default: %{
        max_avg_execution_time: 5_000, # 5 seconds
        max_error_rate: 5.0, # 5%
        max_queue_length: 100,
        min_throughput: 10 # tasks per minute
      }, doc: "Performance thresholds for alerts"],
      trend_analysis_window: [type: :integer, default: 20, 
                             doc: "Number of samples for trend analysis"],
      enable_optimization_suggestions: [type: :boolean, default: true,
  ...
  def mount(config) do
    ...
    initial_state = Map.merge(config, %{
      agent_metrics: %{},
      performance_history: %{},
  ...
  def deliver_signal(state) do
    try do
      ...
      new_state = %{state |
        ...
        performance_history: update_performance_history(state.performance_history, performance_analysis),
        last_analysis: DateTime.utc_now()
      }
  ...
  defp collect_agent_performance_data() do
    try do
      ...
      agent_data = Enum.reduce(registry_entries, %{}, fn {agent_id, pid, metadata}, acc ->
  ...
  defp update_performance_stats(stats, new_data) do
    ...
    %{stats |
      ...
      avg_memory_usage: (stats.avg_memory_usage * stats.total_samples + new_data.memory_mb) / new_sample_count,
      max_memory_usage: max(stats.max_memory_usage, new_data.memory_mb),
      avg_queue_length: (stats.avg_queue_length * stats.total_samples + new_data.message_queue_length) / new_sample_count,
  ...
  defp analyze_agent_performance(agent_metrics, state) do
    ...
    analysis_results = Enum.map(agent_metrics, fn {agent_id, agent_data} ->
      ...
      performance_issues = if stats.avg_memory_usage > 100 do # 100MB threshold
      ...
      performance_issues = if stats.avg_queue_length > thresholds.max_queue_length do
      ...
      performance_issues = if stats.responsiveness_rate < 0.9 do
      ...
      suggestions = if state.enable_optimization_suggestions do
  ...
  defp generate_optimization_suggestions(issues, _stats, trends) do
    ...
    suggestions = if Enum.any?(issues, fn {type, _} -> type == :high_memory_usage end) do
    ...
    suggestions = if Enum.any?(issues, fn {type, _} -> type == :high_queue_length end) do
      ["Consider increasing processing capacity or implementing queue prioritization" | suggestions]
    else
      suggestions
    end
    
    # Responsiveness optimization
    suggestions = if Enum.any?(issues, fn {type, _} -> type == :low_responsiveness end) do
    ...
    suggestions = case Map.get(trends, :memory_trend) do
      :increasing ->
  ...
  defp summarize_performance(analysis) do
    ...
    %{
      ...
      total_issues: Enum.reduce(analysis, 0, fn {_, data}, acc -> 
  ...
  defp get_problem_agents(analysis) do
    ...
    |> Enum.take(10) # Limit to top 10 problem agents
  end
  ...
  defp generate_system_recommendations(analysis, summary) do
    ...
    recommendations = if summary.critical_count > 0 do
      ["Immediate attention required for #{summary.critical_count} critical agents" | recommendations]
    else
      recommendations
    end
    
    recommendations = if summary.degraded_count > summary.normal_count do
    ...
    recommendations = if high_memory_agents > 3 do
