defmodule JidoSystem.Sensors.SystemHealthSensor do
  ...
  use Jido.Sensor,
    ...
    schema: [
      ...
      history_size: [type: :integer, default: 100, 
                    doc: "Number of historical data points to retain"],
      alert_cooldown: [type: :integer, default: 300_000, 
  ...
  def mount(config) do
    ...
    config_with_defaults = Map.merge(%{
    ...
    initial_state = Map.merge(config_with_defaults, %{
      last_metrics: %{},
      metrics_history: [],
      last_alerts: %{},
      baseline_metrics: %{},
  ...
  def deliver_signal(state) do
    try do
      ...
      updated_history = [current_metrics | state.metrics_history]
  ...
  defp collect_process_metrics() do
    %{
      ...
      usage_percent: (:erlang.system_info(:process_count) / :erlang.system_info(:process_limit)) * 100,
  ...
  defp collect_registry_metrics() do
    try do
      %{
        ...
        jido_registry_count: case Process.whereis(Jido.Registry) do
  ...
  defp calculate_memory_usage_percent(memory_info) do
    ...
    if total > 0 do
      ...
      min(100.0, (total / (1024 * 1024 * 1024)) * 10) # Assuming ~10GB system
    else
      0.0
    end
  end
  ...
  defp get_message_queue_stats() do
    try do
      ...
      %{
        ...
        avg: if(length(queue_lengths) > 0, do: Enum.sum(queue_lengths) / length(queue_lengths), else: 0),
  ...
  defp analyze_system_health(current_metrics, history, thresholds, enable_anomaly_detection) do
    ...
    anomalies = if enable_anomaly_detection and length(history) >= 10 do
  ...
  defp check_thresholds(metrics, thresholds) do
    ...
    violations = case get_average_cpu_utilization(metrics.system.scheduler_utilization) do
    ...
    violations = if metrics.memory.usage_percent > memory_threshold do
    ...
    violations = if metrics.processes.count > process_threshold do
    ...
    violations = if metrics.processes.message_queue_lengths.max > queue_threshold do
      [{:message_queue_size, metrics.processes.message_queue_lengths.max, queue_threshold} | violations]
    else
      violations
    end
  ...
  defp detect_anomalies(current_metrics, history) do
    ...
    anomalies = if memory_anomaly do
    ...
    anomalies = if process_anomaly do
  ...
  defp detect_statistical_anomaly(current_value, historical_values) do
    if length(historical_values) >= 10 do
      ...
      variance = Enum.sum(Enum.map(historical_values, fn x -> :math.pow(x - mean, 2) end)) / length(historical_values)
  ...
  defp generate_recommendations(:warning, analysis, _metrics) do
    ...
    recommendations = if Enum.any?(analysis.threshold_violations, fn {type, _, _} -> type == :memory_usage end) do
      ["Consider monitoring memory usage and running garbage collection" | recommendations]
    else
      recommendations
    end
    
    recommendations = if Enum.any?(analysis.threshold_violations, fn {type, _, _} -> type == :process_count end) do
