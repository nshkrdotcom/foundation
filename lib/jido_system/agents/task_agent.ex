  ...
  def mount(server_state, opts) do
    case super(server_state, opts) do
      {:ok, initialized_server_state} ->
        ...
        Logger.info("TaskAgent #{agent.id} mounted with capabilities: #{inspect(get_default_capabilities())}")
      ...
      {:ok, updated_agent} ->
        ...
        new_state = case result do
          ...
          _ ->
            ...
            Logger.debug("TaskAgent received unexpected result format: #{inspect(result, limit: 50, printable_limit: 1000)}")
            %{updated_agent.state | 
              status: :idle,
              current_task: nil
            }
  ...
  def on_error(agent, error) do
    case super(agent, error) do
      {:ok, updated_agent, directives} ->
        ...
        should_count_error = case error do
        ...
        if new_state.error_count >= 10 and should_count_error do
          ...
          final_state = if should_count_error do
            %{new_state | status: :idle}  # Changed from :error to :idle
          else
            new_state
          end
    ...
    try do
      ...
      case ValidateTask.run(task_params, %{}) do
        {:ok, validated_task} ->
          ...
          new_metrics = update_performance_metrics(agent.state.performance_metrics, processing_time, validated_task)
          new_state = %{agent.state | performance_metrics: new_metrics}
          
          emit_event(agent, :task_completed, %{
