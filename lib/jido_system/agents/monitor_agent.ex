defmodule JidoSystem.Agents.MonitorAgent do
  ...
  use JidoSystem.Agents.FoundationAgent,
    ...
    schema: [
      ...
      system_health: [type: :map, default: %{
        overall_status: :unknown,
        score: 0,
        last_assessment: nil
      }],
      monitored_agents: [type: :list, default: []],
      collection_interval: [type: :integer, default: 30_000], # 30 seconds
      history_retention: [type: :integer, default: 1440] # 24 hours worth of data points
    ]
  ...
  def handle_collect_metrics(agent, _params) do
    try do
      ...
      new_history = [metrics | agent.state.metrics_history]
                   |> Enum.take(agent.state.history_retention)
      
      new_state = %{agent.state | metrics_history: new_history}
      
      emit_event(agent, :metrics_collected, %{
        metrics_count: map_size(system_metrics) + map_size(agent_metrics) + map_size(foundation_metrics),
  ...
  def handle_analyze_health(agent, _params) do
    if Enum.empty?(agent.state.metrics_history) do
      ...
      emit_event(agent, :health_analyzed, %{
  ...
  def handle_check_thresholds(agent, _params) do
    if Enum.empty?(agent.state.metrics_history) do
      ...
      {new_alerts, alert_actions} = process_threshold_violations(
        violated_thresholds, 
      ...
      Enum.each(alert_actions, fn {action, alert} ->
        emit_event(agent, :alert_triggered, %{
  ...
  def handle_get_system_health(agent, _params) do
    health_report = Map.merge(agent.state.system_health, %{
  ...
  def handle_set_alert_thresholds(agent, thresholds) when is_map(thresholds) do
    ...
    case validate_thresholds(thresholds) do
      :ok ->
        ...
        emit_event(agent, :thresholds_updated, %{
  ...
  def handle_get_active_alerts(agent, _params) do
    alerts_summary = agent.state.active_alerts
  ...
  defp schedule_health_analysis() do
    Process.send_after(self(), :analyze_health, 60_000) # 1 minute
  end
  ...
  defp subscribe_to_telemetry_events() do
    ...
    Enum.each(events, fn event ->
      Telemetry.attach("monitor_agent_#{inspect(self())}_#{Enum.join(event, "_")}", 
        event, 
  ...
  defp collect_agent_metrics(monitored_agents) do
    agent_stats = Enum.map(monitored_agents, fn agent_id ->
  ...
  defp score_memory_usage(memory_info, threshold) when is_map(memory_info) do
    ...
    if total > 0 do
      usage_percent = (total / (1024 * 1024 * 1024)) * 100 # Convert to GB percentage
  ...
  defp score_foundation_health(%{registry_count: count}) when count >= 0 do
    min(100, count * 10) # Basic scoring
  end
  ...
  defp process_threshold_violations(violations, current_alerts, metrics) do
    ...
    {new_alerts, actions} = Enum.reduce(violations, {current_alerts, []}, fn violation, {alerts, actions} ->
      ...
      alert = %{
        ...
        message: "#{violation.type} exceeded threshold: #{violation.current} > #{violation.threshold}",
  ...
  defp generate_recommendations(metrics, thresholds) do
    ...
    recommendations = if check_memory_threshold(metrics.system.memory, thresholds.memory_usage, :memory_usage) do
    ...
    recommendations = if check_cpu_threshold(metrics.scheduler_utilization, thresholds.cpu_usage, :cpu_usage) do
      ["Consider optimizing CPU-intensive processes or adding more processing capacity" | recommendations]
    else
      recommendations
    end
    
    # Add agent recommendations
    recommendations = if check_agent_threshold(metrics.agents, thresholds.agent_failures, :agent_failures) do
    ...
    case event do
      [:jido_system, :agent, :started] ->
        ...
        if agent_id and agent_id not in state.agent.state.monitored_agents do
          ...
          emit_event(state.agent, :agent_registered_for_monitoring, %{
      ...
      [:jido_system, :agent, :terminated] ->
        ...
        if agent_id do
          ...
          emit_event(state.agent, :agent_deregistered_from_monitoring, %{
            agent_count: length(new_monitored)
          }, %{agent_id: agent_id})
