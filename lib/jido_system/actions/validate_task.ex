defmodule JidoSystem.Actions.ValidateTask do
  ...
  use Jido.Action,
    ...
    schema: [
      ...
      validation_rules: [type: {:list, :atom}, default: [:syntax, :semantics], 
  ...
  def run(params, context) do
    ...
    Logger.info("Starting task validation", task_id: task_id, 
    ...
    try do
      ...
      cached_result = if params.cache_results do
        Cache.get(cache_key)
      else
        nil
      end
      ...
      case cached_result do
        nil ->
          ...
          if params.cache_results and match?({:ok, _}, result) do
            Cache.put(cache_key, result, ttl: 300_000) # 5 minutes
          end
      ...
      e ->
        ...
        :telemetry.execute([:jido_system, :validation, :error], %{
          duration: duration,
          count: 1
        }, %{
          task_id: task_id,
  ...
  defp perform_validation(task, params, context) do
    ...
    final_result = Enum.reduce_while(params.validation_rules, {:ok, validation_results}, 
      fn rule, {:ok, acc} ->
  ...
  defp apply_validation_rule(:syntax, task, _params, _context) do
    ...
    errors = if not Map.has_key?(task, :id) or not is_binary(task.id) do
      ["Missing or invalid task ID" | errors]
    else
      errors
    end
    
    errors = if not Map.has_key?(task, :type) or not is_atom(task.type) do
    ...
    # Input validation
    errors = case Map.get(task, :input) do
    ...
    errors = case Map.get(task, :config) do
      config when is_map(config) -> errors
      nil -> errors # Config is optional
  ...
  defp apply_validation_rule(:semantics, task, _params, _context) do
    ...
    {errors, warnings} = case task_type do
      :data_processing ->
  ...
  defp apply_validation_rule(:business_rules, task, params, context) do
    ...
    rule_results = Enum.map(business_rules, fn {rule_name, rule_config} ->
      apply_business_rule(rule_name, rule_config, task, context)
    end)
    
    failed_rules = Enum.filter(rule_results, fn
      {:error, _} -> true
      _ -> false
    end)
  ...
  defp validate_data_processing_task(input_data, errors, warnings) do
    ...
    errors = if not Map.has_key?(input_data, :source) do
    ...
    warnings = if not Map.has_key?(input_data, :format) do
      ["Consider specifying data format for better processing" | warnings]
    else
      warnings
    end
    
    warnings = if not Map.has_key?(input_data, :validation_rules) do
  ...
  defp validate_validation_task(input_data, errors, warnings) do
    ...
    errors = if not Map.has_key?(input_data, :data) do
      ["Validation task requires 'data' field" | errors]
    else
      errors
    end
    
    warnings = if not Map.has_key?(input_data, :schema) do
  ...
  defp validate_transformation_task(input_data, errors, warnings) do
    ...
    errors = if not Map.has_key?(input_data, :transformations) do
      ["Transformation task requires 'transformations' field" | errors]
    else
      errors
    end
    
    errors = if not Map.has_key?(input_data, :target_format) do
  ...
  defp validate_analysis_task(input_data, errors, warnings) do
    ...
    errors = if not Map.has_key?(input_data, :analysis_type) do
      ["Analysis task requires 'analysis_type' field" | errors]
    else
      errors
    end
    
    warnings = if not Map.has_key?(input_data, :parameters) do
  ...
  defp validate_notification_task(input_data, errors, warnings) do
    ...
    errors = if not Map.has_key?(input_data, :recipients) do
      ["Notification task requires 'recipients' field" | errors]
    else
      errors
    end
    
    errors = if not Map.has_key?(input_data, :message) do
  ...
  defp apply_business_rule(:max_input_size, config, task, _context) do
    max_size = Map.get(config, :max_size, 1_000_000) # 1MB default
    input_data = Map.get(task, :input, %{})
    ...
    if estimated_size <= max_size do
      ...
      {:error, %{rule: :max_input_size, size: estimated_size, limit: max_size, 
  ...
  defp apply_business_rule(:allowed_task_types, config, task, _context) do
    ...
    if Enum.empty?(allowed_types) or task_type in allowed_types do
      ...
      {:error, %{rule: :allowed_task_types, type: task_type, allowed: allowed_types,
  ...
  defp apply_business_rule(:rate_limit, config, _task, context) do
    ...
    window = Map.get(config, :window, 60) # seconds
    limit = Map.get(config, :limit, 100) # tasks per window
    ...
    if current_count < limit do
      ...
      {:error, %{rule: :rate_limit, count: current_count, limit: limit,
  ...
  defp validate_with_external_services(task, validators, timeout) do
    ...
    failed_validations = Enum.filter(results, fn
      {:error, _} -> true
      _ -> false
    end)
  ...
  defp validate_with_schema_service(task, endpoint, _timeout) do
    ...
    if Map.has_key?(task, :id) and Map.has_key?(task, :type) do
      ...
      {:error, %{service: :schema_validator, result: :invalid, 
                 reason: "Missing required fields", endpoint: endpoint}}
    end
  end
  ...
  defp validate_with_data_quality_service(task, endpoint, _timeout) do
    ...
    if quality_score >= 0.7 do
      ...
      {:error, %{service: :data_quality, result: :invalid, score: quality_score,
                 reason: "Data quality below threshold", endpoint: endpoint}}
    end
  end
  ...
  defp validate_with_compliance_service(task, endpoint, _timeout) do
    ...
    if task_type in compliant_types do
      ...
      {:error, %{service: :compliance, result: :non_compliant,
                 reason: "Task type not in compliance list", endpoint: endpoint}}
    end
  end
  ...
  defp calculate_mock_quality_score(input_data) do
    ...
    non_empty_fields = Enum.count(input_data, fn {_, v} -> 
  ...
  defp generate_cache_key(task, validation_rules) do
    task_hash = :crypto.hash(:sha256, :erlang.term_to_binary(task))
                |> Base.encode16(case: :lower)
    rules_hash = :crypto.hash(:sha256, :erlang.term_to_binary(validation_rules))
                 |> Base.encode16(case: :lower)
  ...
  defp emit_validation_telemetry(result, task_id, start_time, context, from_cache) do
    ...
    case result do
      {:ok, _validated_task} ->
        :telemetry.execute([:jido_system, :validation, :completed], %{
          duration: duration,
          count: 1
        }, %{
          task_id: task_id,
      ...
      {:error, error_details} ->
        :telemetry.execute([:jido_system, :validation, :failed], %{
          duration: duration,
          count: 1
        }, %{
          task_id: task_id,
