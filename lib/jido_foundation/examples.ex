defmodule JidoFoundation.Examples do
  ...
  defmodule SimpleJidoAgent do
    ...
    def init(config) do
      ...
      :ok =
        JidoFoundation.Bridge.register_agent(self(),
  ...
  defmodule ExternalServiceAgent do
    ...
    def handle_call({:fetch_data, endpoint}, _from, state) do
      ...
      result =
        JidoFoundation.Bridge.execute_protected(
          self(),
          fn ->
  ...
  def protected_external_calls(num_calls \\ 10) do
    ...
    {:ok, agent} =
      ExternalServiceAgent.start_link(%{
  ...
  def resource_managed_processing(batches) do
    ...
    agents =
      for i <- 1..3 do
        {:ok, agent} =
          ComputeIntensiveAgent.start_link(%{
            max_batch_size: 50,
            agent_id: i
          })
