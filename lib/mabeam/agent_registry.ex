defmodule MABEAM.AgentRegistry do
  ...
  **Write Operations** (`register`, `update_metadata`, `unregister`):
  ...
  def find_by_capability(capability) do
    agent_ids =
      :ets.lookup(:agent_capability_idx, capability)
      |> Enum.map(&elem(&1, 1))
  ...
  def find_by_health_status(health_status) do
    agent_ids =
      :ets.lookup(:agent_health_idx, health_status)
      |> Enum.map(&elem(&1, 1))
  ...
  def find_by_node(node) do
    agent_ids =
      :ets.lookup(:agent_node_idx, node)
      |> Enum.map(&elem(&1, 1))
      ...
      :ok ->
        try do
          ...
          e in [ArgumentError, MatchError] ->
            Logger.warning(
              "Invalid query criteria: #{inspect(criteria)}, error: #{Exception.message(e)}"
            )
  ...
  defp batch_lookup_agents_from_state(agent_ids, state) do
    results =
      agent_ids
  ...
  defp validate_criteria(criteria) do
    invalid_criterion = Enum.find(criteria, &invalid_criterion?/1)

    case invalid_criterion do
      nil -> :ok
      criterion -> {:error, "Invalid criterion format: #{inspect(criterion)}"}
    end
  end
  ...
  defp apply_operation(actual, expected, :eq) do
    # Special handling for capability lists: if actual is a list and expected is an atom,
    # check if the expected capability is in the list
  ...
  # Read operations for protocol support
    ...
    result =
      case attribute do
        :capability ->
          ...
          batch_lookup_agents_from_state(agent_ids, state)
