defmodule JidoSystem do
  ...
  def start_monitoring(opts \\ []) do
    ...
    with {:ok, monitor_agent} <- create_agent(:monitor_agent, monitor_opts),
      ...
      Telemetry.emit(
        [:jido_system, :monitoring, :started],
        %{
  ...
  def execute_workflow(coordinator, workflow, opts \\ []) do
    Logger.info(
      "Executing workflow #{Map.get(workflow, :id)} with coordinator #{inspect(coordinator)}"
    )
  ...
  def get_system_status() do
    try do
      ...
      jido_agents =
        Enum.filter(agent_entries, fn {_, _, metadata} ->
  ...
  defp start_sensors(sensor_types, interval, thresholds) do
    sensor_pids =
      Enum.reduce_while(sensor_types, %{}, fn sensor_type, acc ->
