defmodule MABEAM.Migration do
  ...
  defp get_legacy_agents do
    ...
    try do
      case Process.whereis(MABEAM.ProcessRegistry) do
        nil ->
          {:error, :legacy_registry_not_running}
        ...
        _pid ->
          ...
          try do
            # MABEAM.ProcessRegistry.list_agents() should return {:ok, agents}
            # If it does not exist, the rescue clause will handle it
            MABEAM.ProcessRegistry.list_agents()
          rescue
            _ -> {:ok, []}
          end
      end
      ...
      case MABEAM.Agent.register_agent(unified_config) do
        :ok ->
          if cleanup_old do
            ...
            _ = MABEAM.ProcessRegistry.stop_agent(legacy_agent.id)
          end
  ...
  defp rollback_agent(service_key, metadata, cleanup_unified) do
    ...
    try do
      ...
      case MABEAM.ProcessRegistry.register_agent(legacy_config) do
        :ok ->
  ...
  defp transform_unified_to_legacy(agent_id, metadata) do
    ...
    MABEAM.Types.new_agent_config(
      agent_id,
      metadata[:module] || MABEAM.Agent.Worker,
      metadata[:args] || [],
