defmodule JidoSystem.Agents.TaskAgentTest do
  ...
  describe "task processing" do
    ...
    test "respects paused status", %{agent: agent} do
      ...
      # Wait for cast to be processed using proper wait_for pattern
      wait_for(
        fn ->
          {:ok, state} = Jido.Agent.Server.state(agent)
          state.agent.state.status == :paused
        end,
        1000
      )
  ...
  describe "task queuing" do
    ...
    test "queues tasks with different priorities", %{agent: agent} do
      ...
      # Wait for all casts to be processed and queue to reach expected size
      wait_for(
        fn ->
          case get_queue_size_sync(agent) do
            {:ok, size} when size == 3 -> true
            _ -> nil
          end
        end,
        1000
    ...
    test "respects queue size limits", %{agent: agent} do
      ...
      # Wait for all casts to be processed and queue to stabilize
      wait_for(
        fn ->
          case get_queue_size_sync(agent) do
            {:ok, size} when size <= max_size -> true
            _ -> nil
          end
        end,
        1000
  ...
  describe "performance metrics" do
    ...
    test "provides status information", %{agent: agent} do
      ...
      wait_for(
        fn ->
          {:ok, _state} = Jido.Agent.Server.state(agent)
          true
        end,
        500
      )
