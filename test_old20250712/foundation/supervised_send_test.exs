defmodule Foundation.SupervisedSendTest do
  ...
  describe "broadcast_supervised/3" do
    test "best effort strategy returns all results" do
      ...
      assert {:ok, results} =
               SupervisedSend.broadcast_supervised(
                 recipients,
                 {:broadcast, "test"},
    ...
    test "best effort strategy handles partial failures" do
      ...
      failed_count =
        Enum.count(results, fn {_id, result, _meta} ->
          result != :ok
        end)
    ...
    test "at least one strategy succeeds if one recipient succeeds" do
      ...
      success_count =
        Enum.count(result_list, fn {_id, result, _meta} ->
          result == :ok
        end)
  ...
  describe "telemetry events" do
    test "emits telemetry on successful send" do
      ...
      assert_receive {:telemetry, [:foundation, :supervised_send, :send, :start], _measurements,
                      _metadata},
                     1000

      # Should receive stop event
      assert_receive {:telemetry, [:foundation, :supervised_send, :send, :stop], _measurements,
                      metadata},
                     1000
