defmodule MABEAM.Integration.StressTest do
  use ExUnit.Case, async: false

  alias MABEAM.{AgentRegistry, Types, Comms, Core, Coordination, ProcessRegistry}
    ...
    test "massive concurrent communication load" do
      ...
      agents =
        for i <- 1..agent_count do
          ...
          config = Types.new_agent_config(agent_id, MABEAM.StressTestAgent, agent_id: i)
    ...
    test "hundreds of concurrent coordination sessions" do
      ...
      participants =
        for i <- 1..participant_count do
          ...
          config =
            Types.new_agent_config(agent_id, MABEAM.CoordinationTestAgent, [agent_id: i],
  ...
  describe "memory and performance under stress" do
        ...
        Task.async(fn ->
          agents =
            for i <- 1..200 do
              ...
              config = Types.new_agent_config(agent_id, MABEAM.StressTestAgent, [])
              :ok = ProcessRegistry.register_agent(config)
              {:ok, _pid} = ProcessRegistry.start_agent(agent_id)
              agent_id
            end
    ...
    test "performance degrades gracefully under extreme load" do
      ...
      config = Types.new_agent_config(baseline_agent_id, MABEAM.StressTestAgent, [])
      :ok = ProcessRegistry.register_agent(config)
...
defmodule MABEAM.StressTestAgent do
  use GenServer
...
defmodule MABEAM.CoordinationTestAgent do
  use GenServer
