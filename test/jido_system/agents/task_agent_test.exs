defmodule JidoSystem.Agents.TaskAgentTest do
  ...
  describe "task agent initialization" do
    test "successfully initializes with correct capabilities", %{registry: registry} do
      ...
      case GenServer.whereis(pid) do
        ...
        ^pid -> assert true  # Agent exists and is responsive
      end
    end
  ...
  describe "task processing" do
    ...
    test "respects paused status", %{agent: agent} do
      # Pause the agent
      pause_instruction = Jido.Instruction.new!(%{
  ...
  describe "error handling and recovery" do
    ...
    test "handles processing errors gracefully", %{agent: agent} do
      ...
      error_task = %{
        task_id: "error_task",
        task_type: :invalid_type, # This should cause an error
        input_data: %{},
        timeout: 30_000,
        retry_attempts: 3
      }
      ...
      {:ok, state} = Jido.Agent.Server.state(agent)
      assert state.agent.state.error_count > 0
      assert state.agent.state.status == :idle # Should recover to idle
    end
    
    test "pauses after too many errors", %{agent: agent} do
      # Simulate multiple errors
      for i <- 1..12 do # More than the error threshold (10)
        error_task = %{
      ...
      # Agent should be paused after too many errors
      {:ok, state} = Jido.Agent.Server.state(agent)
