...
defmodule OTPCleanupTestRunner do
  ...
  defp run_suites_parallel(suites, options) do
    ...
    Task.await_many(tasks, 1_200_000)
  end
  ...
  defp generate_report(results, options) do
    ...
    IO.puts(
      "   Success Rate: #{if total_tests > 0, do: Float.round((total_tests - total_failures) / total_tests * 100, 1), else: 0}%"
    )
        ...
        case result.status do
          :failed ->
            ...
            if result.failure_count > 0 do
              IO.puts(
                "     #{result.failure_count} test(s) failed - check implementation correctness"
              )
            end
