defmodule Foundation.PerformanceOptimizer do
  ...
  def generate_recommendations do
    ...
    recommendations =
      if analysis.migration_status.unmigrated_high_risk > 0 do
        [
          "Migrate #{analysis.migration_status.unmigrated_high_risk} high-risk files to UnifiedTestFoundation"
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if analysis.async_potential.can_be_async > 0 do
        [
          "Consider enabling async for #{analysis.async_potential.can_be_async} safe test files"
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if analysis.resource_usage.high_resource_files > 2 do
        [
          "Consider resource pooling for #{analysis.resource_usage.high_resource_files} resource-intensive files"
          | recommendations
        ]
      else
        recommendations
    ...
    async_files =
      ...
      |> Enum.filter(fn file ->
        ...
        String.contains?(content, "async: true") or
          (String.contains?(content, "UnifiedTestFoundation") and
             not String.contains?(content, "async: false"))
      end)
  ...
  defp optimize_async_grouping(sorted_files) do
    ...
    {async_files, sync_files} =
      Enum.split_with(sorted_files, fn %{category: category} ->
