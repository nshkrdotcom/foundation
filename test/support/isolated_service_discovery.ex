defmodule Foundation.IsolatedServiceDiscovery do
    ...
    case validate_supervision_context(supervision_context) do
      :ok ->
        ...
        case Registry.lookup(registry, {:service, service_module}) do
          ...
          other ->
            Logger.warning(
              "Unexpected registry lookup result for #{service_module}: #{inspect(other)}"
            )
    ...
    case validate_supervision_context(supervision_context) do
      :ok ->
        ...
        try do
          ...
          services =
            for {module, pid, metadata} <- service_entries do
              %{
                module: module,
                pid: pid,
  ...
  def call_services_parallel(supervision_context, service_calls, timeout \\ 5000) do
    case validate_supervision_context(supervision_context) do
      :ok ->
        tasks =
          for {service_module, function, args} <- service_calls do
            Task.async(fn ->
  ...
  defp prepare_call_term(call_tuple, args) when is_tuple(call_tuple) and args != [] do
    ...
    Logger.warning(
      "Ambiguous call: both tuple #{inspect(call_tuple)} and args #{inspect(args)} provided. Using tuple."
    )
