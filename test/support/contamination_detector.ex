defmodule Foundation.ContaminationDetector do
  ...
  def analyze_contamination(monitor, final_state) do
    ...
    process_issues = analyze_process_contamination(initial_state.processes, final_state.processes, test_id)
    issues = issues ++ process_issues
    
    # Analyze telemetry contamination
    telemetry_issues = analyze_telemetry_contamination(initial_state.telemetry, final_state.telemetry, test_id)
    ...
    memory_issues = analyze_memory_contamination(initial_state.memory, final_state.memory, monitor.strict_mode)
  ...
  def report_contamination(contamination_report) do
    ...
    IO.puts("═" <> String.duplicate("═", 60))
    IO.puts("ℹ️  Use `use Foundation.UnifiedTestFoundation, :contamination_detection` for automatic cleanup")
    IO.puts("")
  end
  ...
  defp analyze_process_contamination(initial_processes, final_processes, test_id) do
    ...
    issues = if leftover_registered != [] do
      non_test_processes = Enum.reject(leftover_registered, fn name ->
    ...
    issues = if leftover_test_processes != [] do
      [%{
    ...
    issues = if process_growth > 10 do
      [%{
  ...
  defp analyze_telemetry_contamination(initial_telemetry, final_telemetry, test_id) do
    ...
    issues = if leftover_handlers != [] do
      non_test_handlers = Enum.reject(leftover_handlers, fn handler ->
    ...
    issues = if leftover_test_handlers != [] do
  ...
  defp analyze_ets_contamination(initial_ets, final_ets, _test_id) do
    ...
    issues = if leftover_test_tables != [] do
      [%{
  ...
  defp analyze_memory_contamination(initial_memory, final_memory, strict_mode) do
    ...
    {total_threshold, process_threshold} = if strict_mode do
      {5 * 1024 * 1024, 2 * 1024 * 1024}  # 5MB total, 2MB process in strict mode
    else
      {20 * 1024 * 1024, 10 * 1024 * 1024}  # 20MB total, 10MB process in normal mode
    end
    
    issues = []
    
    # Check total memory growth
    issues = if total_growth > total_threshold do
    ...
    issues = if process_growth > process_threshold do
  ...
  defp capture_registry_state(_test_id) do
    ...
    %{
      foundation_registries: [],  # Could capture Foundation registry states
      mabeam_registries: []       # Could capture MABEAM registry states
    }
  end
