defmodule Foundation.OTPCleanupE2ETest do
  ...
  describe "Complete Agent Registration Flow" do
    ...
    test "agent registration, operation, and cleanup with new implementations", %{sup_tree: sup_tree} do
      ...
      operation_span = Span.start_span("agent_lifecycle_test", %{
        test_type: "e2e",
    ...
    test "service integration with supervision tree", %{sup_tree: sup_tree} do
      ...
      {:ok, pool_stats} = call_service(sup_tree, :task_pool_manager, {:get_pool_stats, [:e2e_test_pool]})
  ...
  describe "Complex Workflow Integration" do
    test "multi-step workflow with error handling", %{sup_tree: sup_tree} do
      FeatureFlags.enable_otp_cleanup_stage(4)
      ...
      workflow_span = Span.start_span("multi_step_workflow", %{
        workflow_id: workflow_id,
        total_steps: 5
      })
      ...
      try do
        ...
        agents = for step <- 1..5 do
          ...
          step_span = Span.start_span("create_workflow_agent", %{
            step: step,
            agent_id: agent_id
          })
        ...
        workflow_results = for {step, agent_id, agent_pid} <- agents do
          ErrorContext.set_context(%{
            workflow_id: workflow_id,
            step: "execution",
            current_step: step
          ...
          execution_span = Span.start_span("execute_workflow_step", %{
            step: step,
            agent_id: agent_id
          })
    ...
    test "service restart during workflow", %{sup_tree: sup_tree} do
      ...
      # Register agents that depend on the service
      ...
      {:ok, new_task_manager_pid} = wait_for_service_restart(
        sup_tree, 
        :task_pool_manager, 
        task_manager_pid,
        10_000
  ...
  describe "Production Simulation Tests" do
    test "high load scenario with new implementations", %{sup_tree: sup_tree} do
      FeatureFlags.enable_otp_cleanup_stage(4)
      ...
      tasks = for {agent_id, _agent_pid} <- agents do
        Task.async(fn ->
    ...
    test "memory and resource cleanup", %{sup_tree: sup_tree} do
      FeatureFlags.enable_otp_cleanup_stage(4)
