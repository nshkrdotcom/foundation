defmodule Foundation.MABEAM.ProcessRegistryTest do
  use ExUnit.Case, async: false  # Not async due to registry state
  ...
  setup do
    ...
    configs = [
      Types.new_agent_config(:worker1, Foundation.MABEAM.TestWorker, [], capabilities: [:task_execution]),
      Types.new_agent_config(:worker2, Foundation.MABEAM.TestWorker, [], capabilities: [:data_processing]),
      Types.new_agent_config(:ml_agent, Foundation.MABEAM.MLTestWorker, [], type: :ml_worker, capabilities: [:nlp, :classification])
    ]
  ...
  describe "agent registration" do
    ...
    test "handles registration of agents with complex metadata", %{configs: [base_config, _, _]} do
        ...
        metadata: %{
          ml_model: %{
            ...
            training_data: [
              ...
              %{dataset: "amazon_reviews", size: 100000}
            ]
          },
          deployment: %{
      ...
      retrieved_metadata = case retrieved do
  ...
  describe "agent discovery and search" do
    test "lists all agents", %{configs: configs} do
      ...
      agent_ids = Enum.map(all_agents, fn agent ->
        case agent do
          %{id: id} -> id
          %{config: %{id: id}} -> id
          _ -> nil
    ...
    test "filters agents by status", %{configs: [config1, config2, _]} do
      ...
      case ProcessRegistry.list_agents(status: :running) do
        {:ok, running_agents} ->
          running_ids = Enum.map(running_agents, fn agent ->
            case agent do
              %{id: id} -> id
              %{config: %{id: id}} -> id
              _ -> nil
    ...
    test "searches agents by metadata", %{configs: [base_config, _, _]} do
      ...
      agent2 = %{base_config | 
        id: :search2,
        metadata: %{environment: :staging, team: :backend}
      }
      ...
      case ProcessRegistry.list_agents(metadata: %{environment: :production}) do
        {:ok, prod_agents} ->
          prod_ids = Enum.map(prod_agents, fn agent ->
            case agent do
              %{id: id} -> id
              %{config: %{id: id}} -> id
              _ -> nil
  ...
  describe "error handling and edge cases" do
    ...
    test "handles large numbers of agents", %{configs: [base_config, _, _]} do
      ...
      agent_count = 100  # Reduced for faster tests
      
      _agents = for i <- 1..agent_count do
        config = %{base_config | 
          id: :"agent_#{i}",
          capabilities: [:"capability_#{rem(i, 10)}"]
        }
        :ok = ProcessRegistry.register_agent(config)
        config
      end
    ...
    test "handles concurrent operations safely", %{configs: [base_config, _, _]} do
      ...
      concurrent_agents = Enum.filter(agents, fn agent ->
        agent_id = case agent do
          %{id: id} -> id
          %{config: %{id: id}} -> id
          _ -> nil
        end
    ...
    test "maintains registry consistency under process failures" do
      config = Types.new_agent_config(:failure_test, Foundation.MABEAM.CrashingTestWorker, [crash_after: 50])
    ...
    test "handles malformed agent configurations" do
      malformed_configs = [
        ...
        %{id: :test, module: "not_an_atom"},  # Wrong types
        nil,  # Not a map
  ...
  describe "telemetry and monitoring" do
    test "emits telemetry events for agent lifecycle" do
      ...
      receive do
        {:telemetry, _event} -> :ok
      after
        100 -> :ok  # Telemetry not yet implemented - acceptable
      end
