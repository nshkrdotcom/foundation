defmodule Foundation.MABEAM.TypesTest do
  ...
  describe "agent_config/0" do
    ...
    test "validates agent_id constraints" do
      # Valid IDs
      assert {:ok, _} = Types.validate_agent_config(
        Types.new_agent_config(:valid_id, GenServer, [])
      )
      
      assert {:ok, _} = Types.validate_agent_config(
        Types.new_agent_config(:"agent-123", GenServer, [])
      )
    ...
    property "all agent configs are serializable" do
      check all agent_id <- atom(:alphanumeric),
                module <- member_of([GenServer, Agent, Task]),
                args <- list_of(term(), max_length: 5),
                type <- member_of([:worker, :supervisor, :ml_worker]),
                capabilities <- list_of(atom(:alphanumeric), max_length: 10) do
        
        config = Types.new_agent_config(agent_id, module, args, 
  ...
  describe "coordination_request/0" do
    ...
    property "coordination requests are serializable" do
      check all protocol <- atom(:alphanumeric),
                type <- atom(:alphanumeric),
                params <- map_of(atom(:alphanumeric), term(), max_length: 5) do
  ...
  describe "auction_spec/0" do
    ...
    test "creates valid English auction spec" do
      spec = Types.new_auction_spec(:english, :premium_slot, [:bidder1, :bidder2], 
        starting_price: 10, increment: 1, max_rounds: 10)
    ...
    test "creates valid Dutch auction spec" do
      spec = Types.new_auction_spec(:dutch, :urgent_task, [:agent1], 
        starting_price: 100, decrement: 5, min_price: 20)
    ...
    property "auction specs are serializable" do
      check all type <- member_of([:sealed_bid, :english, :dutch]),
                resource_id <- atom(:alphanumeric),
                participants <- list_of(atom(:alphanumeric), min_length: 1, max_length: 10) do
  ...
  describe "performance characteristics" do
    ...
    test "serialization overhead is minimal" do
      ...
      {time_micro, serialized} = :timer.tc(fn ->
