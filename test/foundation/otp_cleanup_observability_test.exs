defmodule Foundation.OTPCleanupObservabilityTest do
  ...
  describe "Telemetry Event Continuity" do
    setup do
      ...
      handler = fn event, measurements, metadata, _ ->
        send(test_pid, {:telemetry_event, event, measurements, metadata, System.system_time(:millisecond)})
      end
    ...
    test "span parent-child relationships maintained" do
      ...
      :telemetry.attach("span-hierarchy-test", [:foundation, :telemetry, :span, :start], span_handler, nil)
      :telemetry.attach("span-hierarchy-end-test", [:foundation, :telemetry, :span, :end], span_handler, nil)
      ...
      try do
        ...
        collected_span_events = for _i <- 1..8 do  # 4 starts + 4 ends
          receive do
        ...
        start_events = Enum.filter(span_events, fn {event, _, _} -> 
          event == [:foundation, :telemetry, :span, :start] 
        end)
        
        end_events = Enum.filter(span_events, fn {event, _, _} -> 
  ...
  describe "Error Reporting and Context" do
    ...
    test "error context isolation between processes" do
      ...
      error_messages = for _i <- 1..50 do  # Expect ~50 errors (5 per process)
        receive do
  ...
  describe "Performance Monitoring" do
    test "performance metrics collection across implementations" do
      ...
      for {stage, scenario_name, operations} <- test_scenarios do
        ...
        IO.puts("#{scenario_name}: #{Float.round(ops_per_second, 2)} ops/sec, #{Float.round(avg_time_per_op, 2)}μs/op")
      end
    ...
    test "memory usage monitoring" do
      ...
      for field <- memory_fields do
        ...
        max_growth = case field do
          :total -> 200_000_000  # 200MB
          :processes -> 100_000_000  # 100MB
          :atom -> 10_000_000  # 10MB
          :ets -> 50_000_000  # 50MB
        end
  ...
  describe "Observability Integration Tests" do
    test "comprehensive observability workflow" do
      ...
      comprehensive_handler = fn event, measurements, metadata, _ ->
        send(test_pid, {:comprehensive_event, event, measurements, metadata, System.system_time(:microsecond)})
      end
      ...
      :telemetry.attach_many("comprehensive-observability", all_event_patterns, comprehensive_handler, nil)
      ...
      try do
        ...
        workflow_span = Span.start_span("comprehensive_workflow", %{
          workflow_id: workflow_id,
        ...
        for {step, agent_id} <- Enum.with_index(agents, 1) do
          ...
          step_span = Span.start_span("workflow_step", %{
            step: step,
            agent_id: agent_id
          })
        ...
        Process.sleep(200)  # Allow events to be emitted
        ...
        assert length(registry_events) >= 10, "Insufficient registry events: #{length(registry_events)}"
        
        # Should have error context events
        context_events = filter_comprehensive_events(collected_events, [:foundation, :error_context])
        assert length(context_events) >= 5, "Insufficient error context events: #{length(context_events)}"
        
        # Should have span events
        span_events = filter_comprehensive_events(collected_events, [:foundation, :telemetry, :span])
        assert length(span_events) >= 12, "Insufficient span events: #{length(span_events)} (expected >= 12)"
        ...
        assert temporal_variance < 0.1, "Events not in temporal order (variance: #{temporal_variance})"
    ...
    defp calculate_temporal_variance(original, sorted) do
      if length(original) <= 1 do
        ...
        differences = Enum.zip(original, sorted)
