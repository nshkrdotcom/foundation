defmodule MABEAM.AgentRegistryTest do
  ...
  describe "direct ETS lookup operations (lock-free reads)" do
    ...
    test "find_by_capability uses O(1) index lookup", %{
      registry: registry,
      agent1: agent1,
      agent2: agent2
    } do
    ...
    test "find_by_health_status uses O(1) index lookup", %{
      registry: registry,
      agent1: agent1,
      agent2: agent2
    } do
    ...
    test "find_by_node uses O(1) index lookup", %{
      registry: registry,
      agent1: agent1,
      agent2: agent2
    } do
  ...
  describe "atomic multi-criteria queries" do
    ...
    test "query with comparison operators", %{
      ...
      assert {:ok, gte_results} =
               MABEAM.AgentRegistry.query([{[:resources, :cpu_usage], 0.4, :gte}])

      # med_cpu and high_cpu
      assert length(gte_results) == 2

      assert {:ok, lt_results} = MABEAM.AgentRegistry.query([{[:resources, :cpu_usage], 0.6, :lt}])
      # low_cpu and med_cpu
      assert length(lt_results) == 2

      assert {:ok, eq_results} = MABEAM.AgentRegistry.query([{[:resources, :cpu_usage], 0.5, :eq}])
      # med_cpu only
      assert length(eq_results) == 1
    end
  ...
  describe "metadata updates" do
    test "updates metadata and rebuilds indexes", %{registry: registry, agent1: agent1} do
      ...
      {:ok, inference_agents} = MABEAM.AgentRegistry.find_by_capability(:inference)
      # Should be removed from old index
    ...
    test "fails to update metadata for non-existent agent", %{registry: registry} do
      ...
      assert {:error, :not_found} =
               GenServer.call(registry, {:update_metadata, "non_existent", metadata})
    end
  end
    ...
    test "atomic queries are faster than separate operations", %{registry: registry} do
      ...
      metadata_combinations = [
        %{
          ...
          health_status: :healthy,
          node: :node1,
          resources: %{memory_available: 0.8}
        },
        %{
          capability: :training,
          health_status: :healthy,
          node: :node1,
          resources: %{memory_available: 0.6}
        },
        %{
          capability: :inference,
        ...
        %{
          ...
          health_status: :healthy,
          node: :node2,
          resources: %{memory_available: 0.9}
        }
      ]
  ...
  describe "protocol compliance" do
    test "implements Foundation.Registry protocol correctly", %{registry: _registry} do
      ...
      assert {:ok, {^agent_pid, returned_metadata}} =
               Foundation.lookup("protocol_agent", test_registry_name)
